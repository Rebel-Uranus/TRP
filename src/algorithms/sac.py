...
class SAC(object):
	def __init__(self, obs_shape, action_shape, args):
		...
		# print(shared_cnn.out_shape, type(shared_cnn.out_shape)) # torch.Size([32, 21, 21]) <class 'torch.Size'>
	...
	def update_critic(self, obs, action, reward, next_obs, not_done, L=None, step=None):
		...
		# scale = (torch.Tensor([10]).cuda()**torch.floor(torch.log10(torch.abs(critic_loss)))).detach()
		# critic_loss = critic_loss / scale.sum()
	...
	def update_actor_and_alpha(self, obs, L=None, step=None, update_alpha=True):
		...
		# scale = (torch.Tensor([10]).cuda()**torch.floor(torch.log10(torch.abs(actor_loss)))).detach()
		# actor_loss = actor_loss / scale.sum()

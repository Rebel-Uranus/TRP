...
def parse_args():
	...
	# soda
	parser.add_argument('--soda_batch_size', default=128, type=int)
	...
	parser.add_argument('--num_attn_layers', default=2, type=int)
	parser.add_argument('--max_mask_rate', default=0.8, type=float)
	...
	parser.add_argument('--output-dir', type=str, default='results', help='output directory to save GradCAM results')

	args = parser.parse_args()

	assert args.algorithm in {'sac', 'rad', 'curl', 'pad', 'soda', 'drq', 'svea', 'trp'}, f'specified algorithm "{args.algorithm}" is not supported'
	...
	if args.jump <= 0:
		args.jump = int(args.episode_length // 5 // args.action_repeat)

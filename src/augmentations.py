...
def mask_overlay(x, dataset="places365_standard"):
    """Randomly overlay an image from Places"""
    global places_iter
...
def batch_random_conv(x):
	"""Applies a random conv2d, deviates slightly from https://arxiv.org/abs/1910.05396"""
	n, c, h, w = x.shape
	# for i in range(n):
	# k = torch.randint(0, K+1, (1, )).item()
	random_convolution = nn.Conv2d(3, 3, 3, padding=1).to(x.device)
	torch.nn.init.uniform_(random_convolution.weight, 0, 1. / 3)
	temp_x = x.reshape(-1, 3, h, w)/255.
	out = random_convolution(temp_x)*255.

	# weights = torch.randn(n*3, 3, 3, 3).to(x.device)
	# temp_x = x.reshape(-1, 3, h, w)/255.
	# temp_x = F.pad(temp_x, pad=[1]*4, mode='replicate')
	# out = torch.sigmoid(F.conv2d(temp_x, weights))*255.
		# total_out = out if i == 0 else torch.cat([total_out, out], axis=0)
